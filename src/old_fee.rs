        ...
        loop {
            ...
            match received {
                TxOrBlock::Block(block) => {
                    match current_block {
                        Some(current_block) => {
                            ...
                            self.db.put(&block_fee_key(current_block.block_header.bitcoin_hash()), &serialize(&VarInt(block_fee)) ).expect("put error");
                        },
                        None => (),
                    }
                ...
                TxOrBlock::Tx(tx) => {
                    ...
                    match self.db.get(&tx_fee_key).expect("operational problem encountered") {
                        Some(value) => {
                    ...
                    for key in keys {
                        match self.db.get(&key).expect("operational problem encountered") {
                            Some(value) => {
                                let value : VarInt = deserialize(&value).expect("error while deserializing varing");
                                input_sum += value.0;
                            },
                    ...
                    if input_sum > output_sum {
                        ...
                        self.db.put(&tx_fee_key, &serialize(&VarInt(fee))).expect("can't write fee");
                        block_fee += fee;
                    }
                },
                TxOrBlock::End => {
